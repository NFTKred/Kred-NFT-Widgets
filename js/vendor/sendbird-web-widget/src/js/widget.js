...
class SBWidget {
  ...
  _init() {
    ...
    // removed by Jesse Skinner 2018-05-01
    //requestNotification();
	// removed by Angela Ung 2018-05-09
      ...
      this.sb.createHandlerGlobal(
        ...
        (channel, user) => {
          if (this.sb.isCurrentUser(user)) {
            ...
            this.listBoard.setChannelTitle(channel.url, this.sb.getChannelTitle(channel));
            this.updateUnreadMessageCount(channel);
  ...
  createChannelItem(channel) {
    let item = this.listBoard.createChannelItem(
      channel.url,
      channel.coverUrl,
      this.sb.getChannelTitle(channel),
        ...
        this.popup.addClickEvent(this.popup.invitePopup.inviteBtn, () => {
          if (!hasClass(this.popup.invitePopup.inviteBtn, className.DISABLED)) {
            ...
            this.sb.inviteMember(channelSet.channel, selectedUserIds, () => {
              ...
              this.closeInvitePopup();
              this.listBoard.setChannelTitle(channelSet.channel.url, this.sb.getChannelTitle(channelSet.channel));
  ...
  updateChannelInfo(target, channel) {
    this.chatSection.updateChatTop(
      target, this.sb.getMemberCount(channel), this.sb.getChannelTitle(channel)
    );
  }
          ...
          if (event.keyCode == KEY_DOWN_ENTER && !event.shiftKey) {
            ...
            if (!isEmptyString(textMessage.trim())) {
              this.sb.sendTextMessage(channelSet.channel, textMessage, (message) => {
                ...
                // added by Jesse Skinner 2018-05-01
                // we will ask for permission to send notifications only when the user sends a message
                requestNotification();
              });
            }
